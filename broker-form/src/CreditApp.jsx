...
function DOBInp({ value, onChange }) {
  const format = (raw) => {
    ...
    try {
      ...
      const r = await fetch(`${SB_URL}/rest/v1/credit_apps`, {
        ...
        body: JSON.stringify({
          ...
          car_link: f.car_input_type === "link" ? f.car_link.trim() : null,
          car_vin: f.car_input_type === "vin" ? f.car_vin.trim() : null,
      ...
      <motion.div initial={{ opacity: 0, scale: 0.92, y: 24 }} animate={{ opacity: 1, scale: 1, y: 0 }} transition={{ duration: 0.55, ease: [0.4,0,0.2,1] }}
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.45 }}
          ...
          <motion.button whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.97 }} onClick={() => {
            setDone(false);
            setF({ consultant: "", application_type: "lease", car_input_type: "link", car_link: "", car_vin: "", first_name: "", last_name: "", address: "", address2: "", city: "", state: "", zip: "", phone: "", email: "", dob: "", ssn: "", own_or_rent: "", time_at_address: "", monthly_rent: "", employer_name: "", employer_phone: "", employer_address: "", employer_address2: "", employer_city: "", employer_state: "", employer_zip: "", time_in_position: "", job_title: "", annual_income: "", signature_first: "", signature_last: "", signature_canvas: "" });
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.1 }}
          ...
          <div style={{ marginBottom: "20px" }}>
            ...
            <div style={{ display: "flex", gap: "8px", background: C.input, borderRadius: "12px", border: `1px solid ${C.borderSubtle}`, padding: "4px" }}>
              {[["lease", "Lease"], ["finance", "Finance"], ["both", "Lease & Finance"]].map(([val, label]) => {
                const on = f.application_type === val;
                return (
                  <motion.button key={val} whileTap={{ scale: 0.96 }} onClick={() => s("application_type", val)}
                    style={{ flex: 1, padding: "12px 6px", borderRadius: "10px", fontSize: "13px", fontWeight: "600", cursor: "pointer", fontFamily: "inherit", border: "none", background: on ? C.surfaceHover : "transparent", color: on ? C.blue : C.text3, transition: "all 0.2s ease" }}>
                    {label}
                  </motion.button>
                );
              })}
          ...
          {/* Vehicle of Interest */}
          <Section title="Vehicle of Interest" icon="🚗" defaultOpen accent={C.purple}>
            <div style={{ marginBottom: "14px" }}>
              <Label>Link or VIN?</Label>
              <div style={{ display: "flex", gap: "8px", background: C.input, borderRadius: "12px", border: `1px solid ${C.borderSubtle}`, padding: "4px", marginBottom: "12px" }}>
                {[["link", "Car Listing URL"], ["vin", "VIN Number"]].map(([val, label]) => {
                  const on = f.car_input_type === val;
                  return (
                    <motion.button key={val} whileTap={{ scale: 0.96 }} onClick={() => s("car_input_type", val)}
                      style={{ flex: 1, padding: "10px 6px", borderRadius: "10px", fontSize: "13px", fontWeight: "600", cursor: "pointer", fontFamily: "inherit", border: "none", background: on ? C.surfaceHover : "transparent", color: on ? C.purple : C.text3, transition: "all 0.2s ease" }}>
                      {label}
                    </motion.button>
                  );
                })}
              </div>
              {f.car_input_type === "link"
                ? <Inp value={f.car_link} onChange={v => s("car_link", v)} placeholder="https://..." />
                : <Inp value={f.car_vin} onChange={v => s("car_vin", v.toUpperCase().replace(/[^A-Z0-9]/g, "").slice(0, 17))} placeholder="e.g. 1HGBH41JXMN109186" />
              }
            </div>
          </Section>
          ...
          <Section title="Personal Information" icon="👤" defaultOpen accent={C.blue}>
            ...
            <div style={row2}>
              <div><Label required>Date of Birth</Label><DOBInp value={f.dob} onChange={v => s("dob", v)} /></div>
